...
type BoardSharedControlsProps = Omit<
  BoardToolbarProps,
    ...
    <div className="inline-flex items-center gap-1">
      <button
        ...
        className={`inline-flex items-center rounded-md font-medium transition-colors ${
          compact ? "h-7 w-7 justify-center" : "h-6 gap-1 px-2 text-xs"
        } ${
          viewMode === "board"
    ...
    <DropdownMenu>
      <DropdownMenuTrigger
        render={
          <button
            type="button"
            className={`inline-flex items-center rounded-md border border-border bg-background text-foreground font-medium outline-none ring-0 hover:bg-accent/60 ${
              iconOnly ? "h-7 w-7 justify-center" : "h-7 gap-1.5 px-2.5 text-xs"
            }`}
          />
        }
      >
            ...
            <div className="grid grid-cols-1 gap-1 p-1">
              ...
              {project?.columns?.map((column) => (
                <button
                  ...
                  type="button"
                >
                  <CheckSlot checked={selectedStatusIds.includes(column.id)} />
            ...
            <div className="grid grid-cols-1 gap-1 p-1">
              ...
              {["urgent", "high", "medium", "low"].map((priority) => (
                <button
                  ...
                  type="button"
                >
                  <CheckSlot checked={selectedPriorityIds.includes(priority)} />
          ...
          <DropdownMenuSubContent className="w-56">
            <div className="grid grid-cols-1 gap-1 p-1">
              <button
                ...
                type="button"
              >
                <CheckSlot checked={selectedDueDateFilters.length === 0} />
                All due dates
              </button>
      ...
      <div className="hidden border-border/80 border-b bg-card/80 backdrop-blur supports-[backdrop-filter]:bg-card/70 md:block">
        <div className="flex min-h-10 items-center px-2 py-1.5 md:px-3">
          <div className="flex w-full flex-wrap items-center justify-between gap-2">
            ...
            <BoardViewToggle viewMode={viewMode} setViewMode={setViewMode} />
          </div>
        </div>
      </div>

      {hasActiveFilters ? (
        <div className="border-border/80 border-b bg-card/80 px-2 py-2 backdrop-blur supports-[backdrop-filter]:bg-card/70 md:hidden">
